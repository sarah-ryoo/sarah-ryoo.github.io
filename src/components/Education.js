import React from 'react';

const Education = () => {
    return (
        <div className="Education">
            <h2 className="Subheading">Education</h2>
            <div className="Entry">
                    <h2 className="School">British Columbia Institute of Technology</h2>
                    <div className="Degree">Computer Systems Technology Diploma</div>
                    <div className="Experience-description">Graduated with Honours<br/>
                    Predictive Analytics Option:<br/>Predictive Modelling, Machine Learning, Big Data</div>
                    <div className="Degree-content">2019 - 2020</div>
            </div>

            <div class="Entry">
                <h2 class="School">University of Victoria</h2>
                <div class="Degree">Bachelor of Science in Chemistry</div>
                <div class="Degree-content">2010 - 2015</div>
            </div>
        </div>
    );
};

export default Education;